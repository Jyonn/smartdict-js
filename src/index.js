const NOT_FOUND = Symbol("smartdict.notFound");
const UNSET_VALUE = Symbol("smartdict.unsetValue");
const STATUS_RESOLVING = Symbol("smartdict.resolving");
const STATUS_UNRESOLVED = Symbol("smartdict.unresolved");
const STATUS_RESOLVED = Symbol("smartdict.resolved");
...
class Path {
  ...
  child(part) {
    return new Path([...this.parts, String(part)]);
  }
...
class ReferenceNotFoundError extends Error {
  constructor(unresolved) {
    const details = unresolved
      .map((item) => `${item.path || "<root>"} -> ${item.reference}`)
      .join(", ");
...
class SmartDict {
  ...
  resolveRefString(refString, defaultString, path) {
    ...
    if (this.cache.has(refString)) {
      const status = this.cache.get(refString);
      if (status.isResolving) {
